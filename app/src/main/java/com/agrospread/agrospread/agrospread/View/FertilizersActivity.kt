...
import com.agrospread.agrospread.agrospread.ViewModel.FertilizersViewModel
import com.agrospread.agrospread.databinding.ActivityFertilizersBinding
import com.agrospread.agrospread.agrospread.Model.ListData

class FertilizersActivity : AppCompatActivity() {

    private lateinit var binding: ActivityFertilizersBinding
    ...
    private fun setupData() {
        val nameList = arrayOf(
            getString(R.string.product_name_1),
            getString(R.string.product_name_2),
            getString(R.string.product_name_3),
            getString(R.string.product_name_4)
        )
        ...
        val priceList = arrayOf(
            getString(R.string.product_price_1),
            getString(R.string.product_price_2),
            getString(R.string.product_price_3),
            getString(R.string.product_price_4)
        )
    ...
    private fun addToCart(item: ListData) {
        if (viewModel.addItemToCart(item)) {
            Log.d("FertilizersActivity", "Item added to cart: ${item.name}")
            Toast.makeText(this, getString(R.string.toast_item_added_to_cart), Toast.LENGTH_SHORT).show()
        } else {
            Log.d("FertilizersActivity", "Item is already in the cart: ${item.name}")
    ...
    private fun addToFavorites(item: ListData) {
        if (viewModel.addItemToFavorites(item)) {
            Log.d("FertilizersActivity", "Item added to favorites: ${item.name}")
            Toast.makeText(this, getString(R.string.toast_item_added_to_favorites), Toast.LENGTH_SHORT).show()
        } else {
            Log.d("FertilizersActivity", "Item is already in favorites: ${item.name}")
