...
import com.agrospread.agrospread.databinding.ActivityPdetailBinding

class PDetail : AppCompatActivity() {
    private lateinit var binding: ActivityPdetailBinding
    private var quantity: Int = 1
    private var cartItems = ArrayList<ListData>()

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        binding = ActivityPdetailBinding.inflate(layoutInflater)
        ...
        if (intent != null) {
            ...
            val image = intent.getIntExtra("image", R.drawable.p1)
    ...
    private fun addToCart(item: ListData) {
        ...
        if (existingItem != null) {
            ...
            Toast.makeText(this@PDetail, "Item is already in the cart", Toast.LENGTH_SHORT).show()
        } else {
            // Item doesn't exist, add it to the cart
            cartItems.add(item)
            Log.d("PDetail", "Item added to cart: ${item.name}")
            Log.d("PDetail", "Current cart items: $cartItems")
            Toast.makeText(this@PDetail, "Item added to cart", Toast.LENGTH_SHORT).show()
        }
    }
